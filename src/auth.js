...
async function authenticate(req) {
  ...
  if (!key.startsWith(KEY_PREFIX)) {
    throw new ApiError(401, 'invalid_api_key', 'That does not look like a DocMint API key.', {
      hint: 'DocMint keys start with "dm_live_". If you no longer have yours, create a new one on your dashboard at /dashboard — keys are only shown once, so an existing key cannot be read back.',
      docs: '/docs#authentication',
    });
  }
  const { rows } = await query(
