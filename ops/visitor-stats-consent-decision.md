...
| What is stored | Two tables of daily totals only, both in the first-party database. `site_analytics_daily(day, kind, n)` — the pre-existing kind counters (page_view, signup, trial_start, paid_conversion). New: `site_visit_daily(day, path, referrer_host, views, visits)` — `path` has no query string and is limited to the enumerated public pages (an unknown path is **not counted at all**; stricter than the reference's `(unknown route)` bucket, and no such bucket exists here); `referrer_host` is a host name without path or query, empty for own-zone and direct traffic; **"views" = counted full page loads the server served, including own-zone traffic that is not a site entry; "visits" = page loads that entered from outside our own domain zone — this zone-based definition replaces the previous same-site definition — including direct loads with no referrer at all.** The operator report is judged on `visits` and on the delta of the two, never on `views` alone. |
...
| Own hosts | The own domain **zone** rule: the zone is derived once at module load from the canonical `PUBLIC_URL`'s host by taking its last two labels — `mintapis.com` in production. Both live hosts are configured with the same canonical `PUBLIC_URL` (ops/INFRASTRUCTURE.md), so the Render mirror classifies exactly like the canonical host whichever host serves the request; the zone is never derived from the request's Host header, and a missing or malformed `PUBLIC_URL` falls back to the literal zone `mintapis.com`. A referer whose host is the zone itself or **any subdomain of it** — `docmint.app.mintapis.com`, and zone-squatting siblings like `foo.com.mintapis.com` — counts the page view but not a visit and stores no host; lookalikes outside the zone (`mintapis.com.evil.test`, `evilmintapis.com`) stay foreign. On top of the zone rule, the exact hosts of the live Render mirror `docmint-832s.onrender.com` (ops/INFRASTRUCTURE.md: two hosts, both live, one database) plus `localhost` and `127.0.0.1` count as own. **Residual:** sibling subdomains of onrender.com other than our mirror (e.g. `something-else.onrender.com`) still count as foreign visits, with the host stored. |
...
1. `node --test --test-concurrency=1 test/analytics.test.js test/visit-stats.test.js` — the exact
   ...
   reduction with www-strip, truncation, the own-zone rule — sibling subdomain of the zone, bare
   zone, lookalike-foreign hosts, the onrender.com sibling residual — own-host and no-referer
   cases, and the zone derivation with its missing/malformed `PUBLIC_URL` fallback); the
   no-identifier upsert shape (`ON CONFLICT (day, path, referrer_host)`, views=1, visits 0/1) with
   a failed write swallowed; retention deletes on both tables with the 13-month interval and a
   failing delete swallowed; the folded report (rows below 3 and beyond rank 25 in `(other)`,
   zero-visit referrers dropped, days clamped, `unique_visitors: null`); timer idempotence.
...
3. The removal of the two stale "no referrers" claims from `/privacy` (lines ~61 and ~119
   ...
   this specific removal is pinned by that grep here, not by the honesty test. The
   own-domain-zone rule now shipped on `/privacy` is pinned positively by the honesty test
   (the visitor-statistics statement about the own domain zone), so the copy and the code
