...
(async () => {
  ...
  if (!before) throw new Error(`No DocMint account for ${EMAIL}`);
  console.log(`before:  plan=${before.plan} limit=${before.credits_limit}`);

  const customer = await stripe.customers.create({
    email: EMAIL,
    name: 'DocMint webhook verification',
    metadata: { account_id: String(before.id), docmint_verification: 'true' },
  });
  ...
  const sub = await stripe.subscriptions.create({
    ...
    days_until_due: 30,
    metadata: { account_id: String(before.id), plan: 'starter', docmint_verification: 'true' },
  });
  ...
  for (const inv of invoices.data) {
    // Stripe refuses to delete an invoice a subscription created, so a draft one
    // has to be finalised before it can be voided. Leaving it behind would put a
    // stray open invoice on a live account, which is worse than an extra call.
    try {
      if (inv.status === 'draft') {
        const finalised = await stripe.invoices.finalizeInvoice(inv.id);
        await stripe.invoices.voidInvoice(finalised.id);
        console.log(`finalised and voided invoice ${inv.id}`);
      } else if (inv.status === 'open') {
        await stripe.invoices.voidInvoice(inv.id);
        console.log(`voided invoice ${inv.id}`);
      }
    } catch (e) {
      console.warn(`could not clear invoice ${inv.id}: ${e.message}`);
    }
  }
