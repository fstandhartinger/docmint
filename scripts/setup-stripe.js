...
/**
 * Creates (idempotently) the DocMint products, monthly prices and the webhook
...
async function ensureProduct(plan) {
  const found = await stripe.products.search({ query: `metadata['docmint_plan']:'${plan.id}'` });
  if (found.data.length) return found.data[0];
  return stripe.products.create({
    name: `DocMint ${plan.name}`,
    description: `${plan.credits.toLocaleString('en-US')} documents per month`,
    metadata: { docmint_plan: plan.id },
  });
}
...
async function ensureWebhook() {
  ...
  const created = await stripe.webhookEndpoints.create({ url, enabled_events, description: 'DocMint' });
